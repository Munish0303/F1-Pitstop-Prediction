...
```bash
git clone https://github.com/your-username/f1_pitstop_classifier.git
cd f1_pitstop_classifier
```
...
Every `.py` file has a `BASE_DIR` variable near the top. Set it to the absolute path of the project folder on your machine.

Open each file and change this line:

```python
# ── SET THIS TO YOUR PROJECT ROOT ─────────────────────────────────────────────
BASE_DIR = "C:/Users/YourName/f1_pitstop_classifier"   # <-- change this
# ──────────────────────────────────────────────────────────────────────────────
```

**Windows example:**
```python
BASE_DIR = "C:/Users/John/Documents/f1_pitstop_classifier"
```

**Mac / Linux example:**
```python
BASE_DIR = "/home/john/projects/f1_pitstop_classifier"
```

You need to update `BASE_DIR` in all 7 files: `01_preprocess.py`, `02_feature_engineering.py`, `03_train.py`, `04_evaluate.py`, `05_explain.py`, `06_predict.py`, and `run_pipeline.py`.

### 4. Add the data

Place `f1_strategy_v4.csv` inside the `data/` folder:

```
f1_pitstop_classifier/
└── data/
    └── f1_strategy_v4.csv    ← put it here
```
...
This runs all 5 steps in order: preprocess → feature engineering → train → evaluate → explain. Outputs and model files are saved automatically.

```bash
# Predict on real laps from the test set:
python 06_predict.py

# Batch predict on a new CSV:
